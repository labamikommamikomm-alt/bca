    ...
    def report_routes(self, reportname, docids=None, converter=None, **data):
        ...
        if docids:
            try:
                ...
                return request.make_response("Invalid document IDs", status=400)
        ...
        if data.get("cid"):
            try:
                ...
                return request.make_response("Invalid company IDs", status=400)
        ...
        if converter == "html":
            ...
            if docids:
                if len(docids) > 1:
                    model_name = request.env["ir.model"].sudo().get(report.model).name
                    filepart = "{} (x{})".format(model_name, str(len(docids)))
                elif len(docids) == 1:
