...
class RekapHutang(models.TransientModel):
    ...
    def export_excel(self):
        ...
        report_model = self.env['report.breeze_distribution.report_rekap_hutang']
        report_values = report_model._get_report_values(self.ids, data=data)
        lines = report_values.get('lines', [])
        
        output = io.BytesIO()
        workbook = xlsxwriter.Workbook(output, {'in_memory': True})
        sheet = workbook.add_worksheet('Rekap Hutang')
        ...
        # Header
        sheet.merge_range('A1:G1', 'REKAP HUTANG', title_format)
        ...
        # Table Headers
        headers = ['No', 'Tgl Faktur', 'Nomor Faktur', 'Supplier', 'Piutang', 'Terbayar', 'Sisa']
        ...
        for idx, line in enumerate(lines):
            ...
            sheet.write(row, 3, line['supplier'], text_left)
            sheet.write_number(row, 4, line['jumlah'], num_format)
            sheet.write_number(row, 5, line['terbayar'], num_format)
            sheet.write_number(row, 6, line['sisa'], num_format)
            row += 1
            
        # Grand Totals
        sheet.merge_range(row, 0, row, 3, 'GRAND TOTAL', workbook.add_format({'bold': True, 'border': 1, 'align': 'center'}))
        sheet.write_number(row, 4, report_values.get('total_amount', 0), bold_num_format)
        sheet.write_number(row, 5, report_values.get('total_terbayar', 0), bold_num_format)
        sheet.write_number(row, 6, report_values.get('total_sisa', 0), bold_num_format)
