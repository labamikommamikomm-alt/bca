...
class InheritAccountMove(models.Model):
    ...
    sequence_char_doc = fields.Char(
        string="Sequence Doc", compute="compute_sequence_char"
    )
    ...
    def name_get(self):
        ...
        if context.get("special_display_name", False):
            for record in self:
                res.append(
                    (record.id, str(record.name) + " - " + str(record.amount_untaxed))
                )
        else:
            for record in self:
    ...
    def code_inventory(self):
        ...
        pickings = (
            ...
            .sudo()
            .search([("state", "=", "done"), ("origin", "in", sale_order_names)])
        )
    ...
    def no_invoice(self):
        so = self.env["sale.order"].sudo().search([("invoice_ids", "=", self.id)])
        return so.name
    ...
    def _compute_name(self):
        # 1. Separate draft moves (those without a name yet)
        draft_moves = self.filtered(lambda m: m.name == "/")

        # 2. Call super() ONCE on all draft moves to get default sequences
        #    This will assign names like 'MISC/2025/001' and 'INV/2025/001'
        if draft_moves:
            super(InheritAccountMove, draft_moves)._compute_name()

        # 3. Now, iterate and apply custom naming ONLY to specific types
        #    We iterate over 'self' not just 'draft_moves' in case a recompute is triggered
        for move in self:
            # Apply custom name only to invoices/refunds that have a tax number
            if (
                move.move_type in ("out_invoice", "out_refund")
                and move.l10n_id_tax_number
            ):
                try:
                    ...
                    if len(tax_number_clean) >= 8:
                        ...
                        final_invoice_name = (
                            two_digit_year + branch_prefix + eight_digits
                        )

                        # Apply new name (this overrides the default e.g. INV/2025/0001)
                        if move.name != final_invoice_name:
                            move.name = final_invoice_name
                            _logger.info(
                                "Invoice ID %s name overridden to custom format: %s",
                                move.id,
                                final_invoice_name,
                            )
                    else:
                        _logger.warning(
                            "Invoice ID %s: l10n_id_tax_number (%s) too short. Skipping override.",
                            move.id,
                            move.l10n_id_tax_number,
                        )
                except Exception as e:
                    _logger.error(
                        "Error in custom _compute_name for move ID %s: %s", move.id, e
                    )

        # 4. Handle non-draft moves (if any) that were passed to this method
        other_moves = self - draft_moves
        if other_moves:
            super(InheritAccountMove, other_moves)._compute_name()

    # --------------------------------------------------------------------------------
    # (UNCHANGED) action_post
    # --------------------------------------------------------------------------------
    # --------------------------------------------------------------------------------
    # (REVISED) action_post
    # --------------------------------------------------------------------------------
    def action_post(self):
        ...
        for rec in self:
            ...
            if (
                rec.sequence_number_doc == False or rec.sequence_number_doc == 0
            ) and rec.journal_id.type in journal_types_to_sequence:
    ...
    def _get_latest_sequence(self, journal_type):
        ...
        latest_move = self.env["account.move"].search(
            domain, order="sequence_number_doc desc", limit=1
        )
    ...
    def generateGlobalTaxes(self):
        for record in self:
            global_tax_enabled = (
                self.env["ir.config_parameter"].sudo().get_param("account.global_taxes")
                or False
            )
            ...
            record.write({"global_tax_ids": [(5, 0, 0)]})
    ...
    def _generate_efaktur_invoice(self, delimiter):
        ...
        for move in self.filtered(lambda m: m.state == "posted"):
            ...
            eTax["NAMA"] = (
                move.partner_id.nama_ktp
                if eTax["NPWP"] == "000000000000000"
                else move.partner_id.nama_npwp
            )
            eTax["NIK"] = move.partner_id.nik if move.partner_id.nik != False else ""
            eTax["ALAMAT_LENGKAP"] = (
                move.partner_id.alamat_ktp
                if eTax["NPWP"] == "000000000000000"
                else move.partner_id.alamat_npwp
            )
            eTax["JUMLAH_DPP"] = int(
                round(move.amount_untaxed, 0)
            ...
            eTax["UANG_MUKA_PPN"] = int(
                abs(sum(lines.mapped(lambda l: l.price_total - l.price_subtotal)))
            )

            company_npwp = company_id.partner_id.vat or "000000000000000"

            fk_values_list = ["FK"] + [eTax[f] for f in self.FK_HEAD_LIST1[1:]]
            ...
            lt_values_list = ["FAPR", company_npwp, company_id.name] + [
                eTax[f] for f in self.LT_HEAD_LIST1[3:]
            ]
            ...
            for sale in sales:
                of_values_list = (
                    ["OF"]
                    + [str(sale[f]) for f in self.OF_HEAD_LIST1[1:-11]]
                    + ["0", "0", "", "", "", "", "", "", "", "", ""]
                )
