...
class PurchaseOrder(models.Model):
    ...
    is_peminjaman = fields.Boolean(
        string="Peminjaman Barang",
        default=False,
    ...
    def _check_borrow_flag(self):
        for order in self:
            if (
                ...
                and any(line.price_unit != 0 for line in order.order_line)
            ):
...
class PurchaseOrderLine(models.Model):
    ...
    is_peminjaman_line = fields.Boolean(
        related="order_id.is_peminjaman", store=True, readonly=False
    )
